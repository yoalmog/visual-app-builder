...
export const AIBuilderPanel: React.FC = () => {
  ...
  const selectedNode = selectedNodeId ? activePage?.root : null; // simplified lookup
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    if (updated) {
      // Sync updated project with builder store
      useBuilderStore.setState({ project: updated });
    }
  };
  ...
  const handleRollback = () => {
    ...
    if (res.success && res.restoredProject) {
      useBuilderStore.setState({ project: res.restoredProject });
    }
  };

  return (
    <div className="flex flex-col h-full bg-[#0A0D14] border-r border-[#1B1E2B] text-slate-200 text-xs w-[360px] select-none">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        {isGenerating && (
          <div className="p-2.5 rounded-lg bg-[#141824] border border-[#1E2333] space-y-2 animate-pulse">
            <div className="flex items-center justify-between text-[11px] text-indigo-300 font-medium">
              <span>{streamStage || 'AI processing...'}</span>
              <span>{streamPercent}%</span>
            </div>
            <div className="w-full h-1.5 bg-[#1B1E2B] rounded-full overflow-hidden">
              <div
        ...
        {pendingApproval && (
          <div className="p-3 rounded-lg bg-amber-950/40 border border-amber-800/80 text-amber-200 space-y-2">
            ...
            <div className="text-[10px] bg-black/40 p-2 rounded border border-amber-900/50">
              <span className="font-semibold text-amber-400">Operations to Apply ({pendingApproval.operations.length}):</span>
        ...
        {/* Pending Plan Application Banner */}
        {pendingOperations.length > 0 && !pendingApproval && (
          <div className="p-2.5 rounded-lg bg-indigo-950/40 border border-indigo-800/60 text-indigo-200 space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-semibold text-xs text-white">Plan Ready ({pendingOperations.length} Operations)</span>
              <button
                onClick={handleApply}
      ...
      <div className="p-2.5 border-t border-[#1B1E2B] bg-[#0E121B]">
        ...
        <div className="flex items-center justify-between mt-1.5 text-[10px] text-slate-500 px-1">
          <span>Mode: <strong className="text-slate-300 capitalize">{mode}</strong></span>
          <span>Shortcut: <kbd className="bg-[#1C2030] px-1 py-0.5 rounded text-slate-400">Ctrl+K</kbd></span>
        </div>
      </div>
    </div>
  );
